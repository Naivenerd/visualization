...
// MyGraphicsView
MyGraphicsView::MyGraphicsView(int _type, QWidget *parent) : QGraphicsView(parent),
                                                             type(_type)
{
...
void MyGraphicsView::nextAni()
{
    if (aniQueue.size() > 0)
    {
        ...
        curAni = next;
        aniQueue.pop_front();
        connect(next, &QTimeLine::finished, [=]()
                {nextAni(); next->deleteLater(); });
...
MyGraphicsVexItem::MyGraphicsVexItem(QPointF _center, qreal _r, int nameID, QGraphicsItem *parent) : QGraphicsEllipseItem(_center.x() - 0.5, _center.y() - 0.5, 4, 4, parent), center(_center), radius(_r)
{
    id = internalID++;
    nameText = QString::asprintf("Node_%d", nameID);
...
MyGraphicsLineItem::MyGraphicsLineItem(MyGraphicsVexItem *start, MyGraphicsVexItem *end, bool hasDir, QGraphicsItem *parent) : QGraphicsLineItem(parent),
                                                                                                                               hasDirection(hasDir),
                                                                                                                               startVex(start),
                                                                                                                               endVex(end)
{
